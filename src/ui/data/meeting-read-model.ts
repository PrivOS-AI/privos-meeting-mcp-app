...
/**
 * Deletes a meeting's row plus everything it stored in Files.
 *
 * The meeting owns a dedicated folder (`Meetings/<date>-<slug>-<id>`), so a
 * single recursive folder delete removes the whole subtree — the tracked
 * artifacts AND any leftover `audio.part-*` chunks from an interrupted or
 * unprocessed session — and the Hub also purges object storage, embedding/OCR
 * derivatives and share records for everything it removes (verified against
 * `file-management.folders/:folderId` in privos-hub, allowlisted for
 * `files:write`).
 *
 ...
 *
 * All Files cleanup is best-effort — a file/folder already gone, or storage
 * the user lacks rights to, must never block the record delete the user
 * explicitly confirmed. The DB row is removed last regardless.
 */
export async function deleteMeeting(app: McpApp, meeting: MeetingReadModel): Promise<void> {
  let folderDeleted = false;
  if (meeting.folderId) {
    try {
      await app.rest({ method: 'DELETE', path: `file-management.folders/${meeting.folderId}`, query: { recursive: 'true' } });
      folderDeleted = true;
    } catch {
      // Fall back to deleting the known artifacts individually below.
    }
  }

  // No folder (older meetings) or the folder delete failed: remove the tracked
  // artifacts directly. Files already gone return 404, which is swallowed.
  if (!folderDeleted) {
